            ...
            public static void Initialize(AppData appId, InventorySettings inventory, LeaderboardObject[] leaderboards, InputAction[] actions)
            {
                ...
                else
                {
                    ...
                    if (Initialized)
                    {
                        ...
                        if (leaderboards != null)
                        {
                            LeaderboardData.GetAll(leaderboards, result =>
                            {
                                ...
                                && isDebugging)
                                    ...
                                    Debug.LogWarning($"An error occured while loading the leaderboards: {result}");
                        ...
                        if(actions != null)
                        {
                            Debug.Log("Initalizing Steam Input System");
                            var actionColleciton = new (string name, InputActionType type)[actions.Length];
                            for(int i = 0; i < actions.Length; i++)
                                actionColleciton[i] = (actions[i].ActionName, actions[i].Type);
                            
                            if(!Input.Client.Init(actionColleciton))
                            {
                                Debug.LogWarning("Initalization of Input returned an unexpected result!");
                            }
                        }
                    }
                    else
